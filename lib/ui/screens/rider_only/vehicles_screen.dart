...
class VehiclesScreenState extends State<VehiclesScreen> {
          ...
          body: Container(
            ...
            child: ListView(
              children: <Widget>[
                            ...
                            child: TextButton(
                              ...
                              child: Row(
                                ...
                                children: <Widget>[
                                  ...
                                  Row(
                                    children: <Widget>[
                                      (primaryPaymentMethodDetails?['brand'] ==
                                              'Apple Pay'
                                          ? const Image(
                                              image: AssetImage(
                                                  'assets/apple_pay_icon.png'),
                                              height: 18)
                                      ...
                                      Text(
                                          '${capitalizeFirstLetter(primaryPaymentMethodDetails?['brand'])}'
                                          ' ${(primaryPaymentMethodDetails?['last4'] ?? '') != "" ? "••••${primaryPaymentMethodDetails?['last4']}" : ""}'),
                                    ],
                                  ),
                ...
                TextButton(
                  onPressed: () async {
                    if (DateTime.now().difference(lastRequestPress).inSeconds <
                        3) return;
                    setState(() {
                      loading = true;
                    });
                    ...
                    requestMade = true;
                    if (primaryPaymentMethodDetails?['id'] == "apple_pay" ||
                        primaryPaymentMethodDetails?['id'] == "google_pay") {
                      try {
  ...
  static void showVehiclesScreen(BuildContext context, double distanceInMeters,
      double lat, double long, Function resetMap) {
    ...
    showModalBottomSheet(
      ...
      shape: const RoundedRectangleBorder(
        // Optional: to style the top corners
      ...
      builder: (BuildContext context) {
        return FractionallySizedBox(
          heightFactor:
              0.5, // Adjust the height factor as needed, e.g., 0.9 for 90% of screen height
  ...
  Future<void> setCartValues(String label, String size, bool zipXL) async {
    ...
    double amount = await Payment.getAmount(
        zipXL, distanceInMiles, 1); // Assuming this is async
