...
class Map extends StatefulWidget {
  ...
  final MyMarkerReset markerReset;
   final List<LatLng>? initialCoordinates;
    final VoidCallback? onNavigateToMainScreenCalled;
   
  
  const Map({Key? key, required this.markerBuilder, required this.markerReset,  this.initialCoordinates ,
  this.onNavigateToMainScreenCalled})
      : super(key: key);
...
void initState() {
  ...
  if (mounted) {
    positionService.getPosition().then((value) {
      setState(() {
       ...
       if (widget.onNavigateToMainScreenCalled != null) {
         if (widget.initialCoordinates != null) {
      // Print out the destination coordinates if provided
      print('Destination Coordinates: ${widget.initialCoordinates}');
      // Load destination coordinates if provided
      loadDestinationCoordinates(widget.initialCoordinates!);
      print(markers);
    }
        //loadTenderloinCoordinates();
        
    
       widget.onNavigateToMainScreenCalled!();
    }
 ...
 void loadDestinationCoordinates(List<LatLng> coordinates) {
    // Use the provided coordinates to set markers or perform other actions
    // For example:
    setState(() {
      markers.add(
        Marker(
